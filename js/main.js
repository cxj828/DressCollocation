function appStart() {
    ...
    $.event.special.slide = {
        setup: function () {
            ...
            $(this).on(app.evtUp + '.slide', function (e) {
                ...
                if (typeof(startTime) != 'number' || e.timeStamp - startTime > 1000) {
                    return;
                }
                if (typeof(sx) != 'number' || typeof(ex) != 'number' || typeof(sy) != 'number' || typeof(ey) != 'number') {
                    return;
                }
                ex = ex - sx;
                ey = ey - sy;
    ...
    function loadingShow(callback) {
        var html = $(".loading");
        if (html.length) {
            html.removeClass('loading_out').off('webkitAnimationEnd');
            return;
        }
        ...
        html.one('webkitAnimationEnd', function () {
            callback();
        });
    };
    ...
    function slideTo(el, isBack, callback) {
        ...
        next.css({
            display: 'block',
            y: isBack ? '-100%' : '100%'
        }).addClass('no_animation').transit({y: 0}, 300, function () {
    ...
    $(".s2 ol").on(app.evtClick, 'li a', function () {
        var li = $(this).parent();
        if (li.is('.curr')) {
            $(".s2 footer .btn1").trigger(app.evtClick);
            return;
        }
    ...
    $(".s2 menu").on('click', 'li', function () {
        ...
        var action = function () {
            ...
            if (type == 1 || type == 4) {
                p = $(".s2 figure .item5");
                if (p[0].preZindex) {
                    p.css('z-index', app.poster.exLevel[4][imgID] && app.poster.exLevel[4][imgID] > p[0].preZindex ? app.poster.exLevel[4][imgID] : p[0].preZindex);
                    p[0].preZindex = undefined;
                }
            ...
            if (type == 1) {
                if (app.poster.exCost[imgID]) {
                    $.each(app.poster.exCost[imgID], function (i, o) {
                        ...
                        if (!o.isoff) {
                            about.children('li').addClass('off');
                        }
    ...
    function createPoster(callback) {
        ...
        action = function () {
            loadingHide();
            setTimeout(function () {
                typeof(callback) == 'function' && callback(poster, poster2, txt);
            }, 100);
        };
        canvas = document.createElement('canvas');
    ...
    $(".s3>center a").on(app.evtClick, function () {
        ...
        app.poster.name = $(".s3>center input:eq(0)").val().trim();
        if (!app.poster.name) {
            myAlert('请填写您的名字！');
            return;
        }
        ;
        if ($(".s3 figure .name font:eq(0)").text() != app.poster.name) {
            app.poster.wordCheck = false;
        }
        ;
        $(".s3 figure .name font:eq(0)").text(app.poster.name);
        $(".s3>center").hide();
    });
    $(".s3 footer h3 a").on(app.evtClick, function () {
        ...
        if (!app.poster.wordCheck) {
            ...
            checkWord(app.poster.name, function (r) {
                if (!r) {
                    loadingHide();
                    myAlert('输入内容检查错误，请重试！');
                    return;
                }
                if (r != 1) {
                    loadingHide();
                    myAlert('您输入的名字违规，请重新输入！');
                    return;
                }
                app.poster.wordCheck = true;
                createPoster(function (poster, poster2, txt) {
    ...
    $(".s4 .btn8").on(app.evtClick, function () {
        $(".desc").css({display: 'block', y: '8%', opacity: 0}).transit({y: 0, opacity: 1}, 200, function () {
